...
# path must be changed if on a different computer than Will's
path = '/Users/WillC/Documents/Code/python/Project_Log_Manager/';
# application is to open the logs is up to you
...
def changePath(currPath):
    try:
        ...
        if (newPath[-1] != '/'):
            newPath += '/';
        print(
        "Remember to change the path variable in the beginning of this code: \nFrom\n" + path + "\nTo this\n" + newPath);
        if (newPath == "0"):
            print("\nWarping...");
            sys.exit();
        changePath(newPath);
...
def getProjNameandPath(dataBase):
    ...
    if (str(projectPick) == "0"):
        ...
        print(
        "Your input was totally invalid and not one of the options specified, man. I am teleporting you back to the selection screen.");
    ...
    if (showOptions):
        ...
        if (str(projectPick) == str(countProject + 3)):  # Change a project's current day
            ...
            pickDay = input("\nWhat would you like to update the log day to for " + dataBase[int(projectPick) - 1][
            ...
            if (logNumChangeValid(projectPick, int(pickDay), dataBase) == False):
                print(
                "This log day has already been created. Please choose another one.\nTeleporting you back to the selection, sir.");
                return getProjNameandPath(dataBase);
    ...
    logPath = dataBase[int(projectPick) - 1][2];
    return [name, currDay, logPath];
...
def checkFullNameExist(name, logPath):
    index = name.index("_");
    logCheck = 0;
    dateCheck = 0;
...
def getNumDay_LogName_checkExist(getProjNameFunction, date, dataBase, logPath):
    rowCount = 0;
    numDay = 0;
    ...
    check = checkFullNameExist(logName, logPath);
...
def createFirstLogBool(projPath, logPath):
    os.chdir(projPath);
    for i in os.listdir(logPath):
        if (i[:3] == "Log"):
...
def copyLastFile(prevDays, copyAfterThis, secondsDay, numDay, logPath):
    count = 0
    for i in os.listdir(logPath):
        if re.search(".txt", i):
            count += 1;
    ...
    while (prevDays < count):
        ...
        prevDays += 1;7
        if (checkFullNameExist(getLogName(str(int(numDay)-1), yesterday), logPath) > 0):
            numDay = str(int(numDay) - 1);
            for i in os.listdir(logPath):
                if re.search("Log"+numDay, i):
                    prevlog = i
            exists = True;
            break;
...
def createLog(dataBase):
    ...
    numDay_LogName_checkExist = getNumDay_LogName_checkExist(projNameandPath, date, currDataBase, logPath);
    numDay, logName, checkName = numDay_LogName_checkExist[0], numDay_LogName_checkExist[1], numDay_LogName_checkExist[
        2];
    ...
    # If this is the first ever log in the series, create a new one
    elif (createFirstLogBool(projNameandPath[2], logPath) == True):
        ...
        progress = "\n\nProgress:";
        todo = "\n\nTo-do:" + copyLastFile(prevDays, "To-do:", secondsDay, numDay, logPath);
